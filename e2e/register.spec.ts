...
test.describe('Testing the register page', () => {
   test.describe('Error states', () => {
      ...
      test('Should show an error message when a username is too short', async ({
         ...
         await page.getByRole('button', { name: /Register Account/i }).click()
         await expect(
            page.getByText('Username must be at least 10 characters', {
               exact: true,
            })
         ).toBeVisible()
      })
      ...
      test('Should show an error if the username is taken', async ({
         ...
         await page.getByRole('button', { name: /Register Account/i }).click()
         await expect(
            page.getByText('User with this username already exists', {
               exact: true,
            })
         ).toBeVisible()
      })
      ...
      test("Should show an error if user's password and confirm password do not match", async ({
         ...
         await page
            ...
            .fill('Danex20')
         await page
            .getByLabel('E-mail (required)', { exact: true })
            .fill('dane20@gmail.com')
         await page
            .getByLabel('Password (required)', { exact: true })
            .fill('thisisapassword')
         await page
            .getByLabel('Confirm Password (required)', { exact: true })
            .fill('thisis')
         await page.getByRole('button', { name: /Register Account/i }).click()
